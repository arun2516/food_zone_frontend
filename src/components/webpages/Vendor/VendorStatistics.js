    ...
    useEffect(() => {
        if (user_type() === 'vendor') {
            async function fetchData() {
                const response_1 = await axios.get('https://foodie-zonee.herokuapp.com/orders/vendor', {
                    headers: {
                        authorization: localStorage.getItem('token')
                    }
                });
                const response_2 = await axios.get('https://foodie-zonee.herokuapp.com/items', {
                    headers: {
                        authorization: localStorage.getItem('token')
                    }
                });
                const response_3 = await axios.get('https://foodie-zonee.herokuapp.com/vendors/details', {
                    headers: {
                        authorization: localStorage.getItem('token')
                    }
                });
                const response_4 = await axios.get('https://foodie-zonee.herokuapp.com/buyers', {
                    headers: {
                        authorization: localStorage.getItem('token')
                    }
                });
